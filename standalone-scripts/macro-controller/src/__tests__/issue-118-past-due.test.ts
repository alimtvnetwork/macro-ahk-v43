 ...
 *   - isExpiringWs filter predicate
 *   - expiring sort (daysPassed desc, available desc tie-break)
 *   - ws-hover-card past-due section
 */
...
import { buildStatusPillHtml } from '../workspace-status';
...
describe('Issue 118 — getEffectiveStatus for past_due', () => {
  it('past_due → past-due-expiring with correct daysSince', () => {
    const ws = makeWs({
      ...
      subscriptionStatusChangedAt: '2026-04-18T00:00:00Z',
    });
  ...
  it('unpaid → past-due-expiring', () => {
    const ws = makeWs({
      ...
      subscriptionStatusChangedAt: '2026-04-15T00:00:00Z',
    });
    const s = getEffectiveStatus(ws, CFG, NOW);
    expect(s.kind).toBe('past-due-expiring');
  ...
  it('past_due ignores refill dates completely', () => {
    const ws = makeWs({
      subscriptionStatus: 'past_due',
      subscriptionStatusChangedAt: '2026-04-20T00:00:00Z',
      nextRefillAt: '2026-04-23T00:00:00Z',
    });
    const s = getEffectiveStatus(ws, CFG, NOW);
    expect(s.kind).toBe('past-due-expiring');
...
describe('Issue 118 — classifyWorkspaceDisplayStatus past-due', () => {
  ...
  it('produces Expire + Passed Nd for daysSince>0', () => {
    const ws = makeWs({
      subscriptionStatus: 'past_due',
      subscriptionStatusChangedAt: new Date(NOW - 6 * 86_400_000).toISOString(),
    });
    const d = classifyWorkspaceDisplayStatus(ws, CFG, NOW);
    expect(d.label).toBe('Expire');
...
describe('Issue 118 — buildStatusPillHtml', () => {
  it('renders main label + sublabel for past-due-expiring', () => {
    const ws = makeWs({
      subscriptionStatus: 'past_due',
      subscriptionStatusChangedAt: new Date(NOW - 3 * 86_400_000).toISOString(),
    });
...
/* ------------------------------------------------------------------ */
/*  Filter predicate (via classifyFromStatus)                            */
/* ------------------------------------------------------------------ */

describe('Issue 118 — isExpiringWs predicate', () => {
  it('past_due workspace classifies as past-due-expiring', () => {
  ...
  it('active workspace does NOT classify as past-due-expiring', () => {
    const ws = makeWs({ subscriptionStatus: 'active' });
    const display = classifyWorkspaceDisplayStatus(ws, CFG, NOW);
    expect(display.kind).toBe('normal');
  });

  it('canceled workspace does NOT classify as past-due-expiring', () => {
    const ws = makeWs({ subscriptionStatus: 'canceled', subscriptionStatusChangedAt: new Date(NOW).toISOString() });
    const display = classifyWorkspaceDisplayStatus(ws, CFG, NOW);
    expect(display.kind).toBe('canceled');
  });
});
...
describe('Issue 118 — subscription-status enum helpers', () => {
  it('isPastDueStatus matches past_due and unpaid', () => {
    const { isPastDueStatus } = await import('../types/subscription-status');
