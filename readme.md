...
&#x200B;<!-- Build & Release --> [![CI](https://img.shields.io/github/actions/workflow/status/alimtvnetwork/macro-ahk-v41/ci.yml?branch=main&label=CI&logo=github&style=flat-square)](https://github.com/alimtvnetwork/macro-ahk-v41/actions/workflows/ci.yml) <!-- Repo activity --> [![Issues](https://img.shields.io/github/issues/alimtvnetwork/macro-ahk-v41?style=flat-square&logo=github)](https://github.com/alimtvnetwork/macro-ahk-v41/issues) [![Pull Requests](https://img.shields.io/github/issues-pr/alimtvnetwork/macro-ahk-v41?style=flat-square&logo=github)](https://github.com/alimtvnetwork/macro-ahk-v41/pulls) [![Repo Size](https://img.shields.io/github/repo-size/alimtvnetwork/macro-ahk-v41?style=flat-square)](https://github.com/alimtvnetwork/macro-ahk-v41)
&#x200B;<!-- Community --> <!-- (intentionally empty — see mem://constraints/no-static-mockup-badges) --> <!-- Code-quality --> [![Security Issues](https://img.shields.io/github/issues-search/alimtvnetwork/macro-ahk-v41?query=is%3Aissue%20label%3Asecurity&label=security&style=flat-square&logo=github)](https://github.com/alimtvnetwork/macro-ahk-v41/issues?q=is%3Aissue+label%3Asecurity) [![Dependency PRs](https://img.shields.io/github/issues-pr-raw/alimtvnetwork/macro-ahk-v41?label=dependency%20PRs&style=flat-square&logo=dependabot)](https://github.com/alimtvnetwork/macro-ahk-v41/pulls?q=is%3Apr+label%3Adependencies) <!-- Stack & standards --> [![License](https://img.shields.io/github/license/alimtvnetwork/macro-ahk-v41?style=flat-square)](#license)
...
```powershell
irm https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v41/main/scripts/download-extension.ps1 | iex
```

**Windows · PowerShell — pin a version + custom folder name (env-var form, works with `irm | iex`):**

```powershell
$env:MARCO_DL_VERSION='v3.31.0'; $env:MARCO_DL_FOLDER='marco'; irm https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v41/main/scripts/download-extension.ps1 | iex
```
...
```powershell
irm https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v41/main/scripts/install.ps1 | iex
```

### 🐧 macOS · Linux · Bash installer (latest)

```bash
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v41/main/scripts/install.sh | bash
```
...
```powershell
# Windows · PowerShell
irm https://github.com/alimtvnetwork/macro-ahk-v41/releases/download/v3.31.0/install.ps1 | iex
```

```bash
# macOS · Linux · Bash
curl -fsSL https://github.com/alimtvnetwork/macro-ahk-v41/releases/download/v3.31.0/install.sh | bash
```
...
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v41/main/scripts/install.ps1))) -InstallDir (Join-Path $PWD 'marco-extension')
```

```bash
# Bash (Linux / macOS) — installs to ./marco-extension under the current folder
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v41/main/scripts/install.sh | bash -s -- --dir "$PWD/marco-extension"
```
...
```powershell
& { $Version = "v2.220.0"; irm https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v41/main/scripts/install.ps1 | iex }
```

```bash
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v41/main/scripts/install.sh | bash -s -- --version v2.220.0
```
...
| `-Repo` | Override GitHub `owner/repo` | `-Repo alimtvnetwork/macro-ahk-v41` |
...
| `--repo <o/r>` | `-r` | Override GitHub `owner/repo` | `--repo alimtvnetwork/macro-ahk-v41` |
...
When the project ships successive major rewrites in sibling repos (e.g. `macro-ahk-v41` → `macro-ahk-v41` → `macro-ahk-v41`), discovery mode can optionally probe ahead and pick the newest. Defaults live in [`scripts/install.config.sh`](scripts/install.config.sh) and are **off by default**:
...
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v41/main/scripts/install.sh \
  | bash -s -- --dry-run
...
1. Download `marco-extension-v{VERSION}.zip` from [Releases](https://github.com/alimtvnetwork/macro-ahk-v41/releases)
...
This creates a `macro-ahk-v41/` folder containing the AHK v2 scripts that pair with the Chrome extension's macro controller.
...
**No email or notification is sent** — check the [Releases page](https://github.com/alimtvnetwork/macro-ahk-v41/releases) for status.
...
```powershell
irm https://github.com/alimtvnetwork/macro-ahk-v41/releases/download/v{VER}/install.ps1 | iex
```

**Linux / macOS**

```bash
curl -fsSL https://github.com/alimtvnetwork/macro-ahk-v41/releases/download/v{VER}/install.sh | bash
```
